...
public:
	...
	void Draw(Shader* shaderT) {
		//shaderT->use();
		shaderT->setMat4("model", trasfMatrix);
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, texture);
		shaderT->setInt("texture_diffuse1", 0);
	...
	vector<TriangoloPt> convertMapToTrPt() {
		...
		for (int i = 0; i < mappaTriangoloVertici.size(); i++) {
			vector<Vertex> listaVertTriangolo = mappaTriangoloVertici[i];
	...
	vec3 updateCameraPositionOnMap(glm::vec3 posCamera, glm::vec3 posVecchia, float offset, bool tutti) {
		...
		if (tutti == true) {
			vicino = alberoKD.getPuntiEntroRaggio(puntoCamera, 800);
		}
		else {
			vicino = alberoKD.getPuntiEntroRaggio(puntoCamera, 250);
		}
		for (int i = 0; i < vicino.size(); i++) {
			...
			if (isPointInsideTriangle(vec3(posCamera.x, y, posCamera.z), vicino[i].listaVertici[0], vicino[i].listaVertici[1], vicino[i].listaVertici[2])) {
	...
	bool updateCameraPositionOnMapCollision(vec3 posCamera, vec3 posVecchia, float offset, bool tutti) {
		if (posCamera.x < minMaxValues.minX || posCamera.x > minMaxValues.maxX || posCamera.z < minMaxValues.minZ || posCamera.z > minMaxValues.maxZ) {
			return true;
		}

		TriangoloPt puntoCamera(posCamera);
...
private:
	...
	vector<Vertex> getVerticesFromMap() {
